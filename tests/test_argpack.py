...
def test_argpack_input(default_os_env):
    ...
    codepack = CodePack(name='argpack_test', code=code1, subscribe=code2)
...
def test_argpack_str(default_os_env):
    ...
    codepack = CodePack(name='argpack_test', code=code1, subscribe=code2)
    argpack = codepack.make_argpack()
    assert argpack.__str__() == 'ArgPack(name: argpack_test, args: {add2(a=None, b=None), mul2(a=None)})'
    argpack['add2'](a=3, b=2)
    argpack['mul2'](c=5)
    argpack['mul2'](a=2)
    assert argpack['add2']['a'] == 3
    assert argpack['add2']['b'] == 2
    assert argpack['mul2']['a'] == 2
    assert argpack.__str__() == 'ArgPack(name: argpack_test, args: {add2(a=3, b=2), mul2(a=2, c=5)})'
    with pytest.raises(TypeError):
        codepack(argpack=argpack)
    assert codepack.get_state() == 'ERROR'
    assert codepack.get_message() == {'mul2': "mul2() got an unexpected keyword argument 'c'"}
...
def test_default_load(default_os_env):
    ...
    assert isinstance(search_result[0], ArgPack) and search_result[0].get_name() == 'test_codepack'
    search_result = ArgPack.load('dummy')
    assert search_result is None
    search_result = ArgPack.load('test_codepack')
    assert search_result is not None
    assert isinstance(search_result, ArgPack) and search_result.get_name() == 'test_codepack'
...
def test_remove(default_os_env):
    ...
    assert ret.get_name() == argpack.get_name()
...
def test_argpack_version(default_os_env):
    ...
    code1 >> code2
    codepack = CodePack(name='test_codepack@1.1.1', code=code1, subscribe=code2)
    argpack1 = codepack.make_argpack()
    assert argpack1.get_name() == 'test_codepack@1.1.1'
    assert argpack1.get_version() == '1.1.1'
    assert argpack1.__str__() == 'ArgPack(name: test_codepack@1.1.1, args: {add2@0.1.1(a=None, b=None)'\
                                 ', haha@1.2.3(a=None, b=None)})'
    argpack1.set_version('4.5.6')
    assert argpack1.__str__() == 'ArgPack(name: test_codepack@4.5.6'\
                                 ', args: {add2@0.1.1(a=None, b=None), haha@1.2.3(a=None, b=None)})'
    argpack2 = ArgPack()
    argpack3 = ArgPack(name='test')
    argpack4 = ArgPack(name='test@1.2.3')
    argpack5 = ArgPack(version='1.1.1')
    assert argpack2.__str__() == 'ArgPack(name: None, args: {})'
    assert argpack3.__str__() == 'ArgPack(name: test, args: {})'
    assert argpack4.__str__() == 'ArgPack(name: test@1.2.3, args: {})'
    assert argpack5.__str__() == 'ArgPack(name: None, args: {})'
