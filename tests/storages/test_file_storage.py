...
def test_file_storage(testdir_file_storage, dummy_deliveries):
    ...
    assert sorted(os.listdir(testdir_file_storage)) == sorted(['%s.json' % d.get_id() for d in dummy_deliveries])
    ...
    assert len(search_result) == 1 and search_result[0].get_id() == dummy_deliveries[0].get_id()
    ...
    for i in range(len(load_result)):
        assert load_result[i].get_id() == dummy_deliveries[i].get_id()
...
def test_file_storage_list_all(testdir_file_storage, dummy_deliveries):
    ...
    assert tmp == sorted(['%s.json' % d.get_id() for d in dummy_deliveries])
    dummy_keys = [d.get_id() for d in dummy_deliveries]
    ...
    assert sorted([d.get_id() for d in all_items]) == sorted(all_keys)
