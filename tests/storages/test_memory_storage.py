...
def test_memory_storage(dummy_deliveries):
    storage = MemoryStorage(item_type=Delivery, key='_name')
    assert storage.key == '_name'
    storage.save(item=dummy_deliveries)
    assert sorted(storage.memory.keys()) == sorted([d.get_name() for d in dummy_deliveries])
    ...
    assert search_result[0].get_name() == dummy_deliveries[0].get_name()
    assert search_result[0].get_serial_number() == dummy_deliveries[0].get_serial_number()
    ...
    for i in range(len(load_result)):
        assert load_result[0].get_name() == dummy_deliveries[0].get_name()
        assert load_result[0].get_serial_number() == dummy_deliveries[0].get_serial_number()
...
def test_memory_storage_list_all(dummy_deliveries):
    ...
    assert storage.key == '_name'
    storage.save(item=dummy_deliveries)
    dummy_keys = sorted([d.get_name() for d in dummy_deliveries])
    ...
    assert sorted([d.get_name() for d in all_items]) == all_keys
