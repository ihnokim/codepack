...
class FileSnapshotService(SnapshotService, FileStorage):
        ...
        if isinstance(snapshot, self.obj):
            ...
            if d:
                ...
                for key, value in existing_snapshot.diff(snapshot).items():
                    ...
                    existing_snapshot.to_file(self.obj.get_path(serial_number=existing_snapshot.serial_number, path=self.path))
            else:
                snapshot.to_file(self.obj.get_path(serial_number=snapshot.serial_number, path=self.path))
        else:
    ...
    def load(self, serial_number, projection=None):
        if isinstance(serial_number, str):
            ret = None
            try:
                d = self.obj.from_file(self.obj.get_path(serial_number=serial_number, path=self.path)).to_dict()
                if projection:
    ...
    def search(self, key, value, projection=None):
        ...
        for filename in glob(self.path + '*.json'):
            snapshot = self.obj.from_file(filename)
