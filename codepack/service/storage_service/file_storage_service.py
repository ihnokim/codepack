...
class FileStorageService(StorageService, FileStorage):
    ...
    def save(self, item, update=False):
        if isinstance(item, self.item_type):
            if update:
                item.to_file(self.item_type.get_path(key=item.id, path=self.path))
    ...
    def load(self, id):
        return self.item_type.from_file(path=self.item_type.get_path(key=id, path=self.path))

    def remove(self, id):
        os.remove(path=self.item_type.get_path(key=id, path=self.path))

    def check(self, id):
        if isinstance(id, str):
            ret = None
            try:
                ret = self.item_type.from_file(path=self.item_type.get_path(key=id, path=self.path)).id
            finally:
                return ret
